...
Texture2D^ ScratchImage::CreateTexture(Device^ device)
{
	...
	auto deviceRaw = static_cast<ID3D11Device*>(device->NativePointer.ToPointer());
	...
	return gcnew Texture2D(System::IntPtr(texture));
}
...
//--------------------------------------------------------------------------------------------------
ScratchImage^ DirectXTexSharp::DirectXTex::Create2D(SharpDX::DataRectangle^ data, int width, int height, SharpDX::DXGI::Format format)
{
	...
	image.slicePitch = 0;
	image.pixels = static_cast<uint8_t*>(data->DataPointer.ToPointer());
