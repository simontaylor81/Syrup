...
namespace ShaderEditorApp.Rendering
{
	...
	class ShaderVariable : IShaderVariable
	{
		...
		internal static PropertyViewModel CreateViewModel(IShaderVariable variable)
		{
			...
			if (Class == ShaderVariableClass.Vector || Class == ShaderVariableClass.Scalar || Class == ShaderVariableClass.MatrixColumns)
			{
				if (Type == ShaderVariableType.Float)
					return new VectorShaderVariableViewModel<float>(variable);
			}

			// TEMP fallback.
			return new SimpleScalarProperty<float>(variable.Name, 0.0f, false);
		}
	}
}
